...
class TCBlock(nn.Module):
    def __init__(self, in_channels, seq_length, filters):
        ...
        self.dense_blocks = nn.ModuleList([DenseBlock(in_channels + i * filters, 2 ** i, filters)
...
class AttentionBlock(nn.Module):
    ...
    def forward(self, input):                         # input: (N, T, C)
        N, T, _ = input.size()

        # ❶ 입력과 **같은 디바이스**·dtype=bool 로 마스크 생성
        mask = torch.triu(
            torch.ones(T, T, dtype=torch.bool, device=input.device),
            diagonal=1            # 상삼각 부분만 1
        )

        keys   = self.linear_keys(input)              # (N, T, K)
        query  = self.linear_query(input)             # (N, T, K)
        values = self.linear_values(input)            # (N, T, V)

        attn_scores = torch.bmm(query, keys.transpose(1, 2))   # (N, T, T)
        attn_scores.masked_fill_(mask, float('-inf'))          # ❷ bool-mask 사용
        attn_weights = F.softmax(attn_scores / self.sqrt_key_size, dim=-1)

        context = torch.bmm(attn_weights, values)     # (N, T, V)
        return torch.cat((input, context), dim=2)     # (N, T, C+V)