...
def get_project_root() -> Path:
    # Assume this file is located at: Pathway_MAML/model/train.py
    # Then project root is: Pathway_MAML
...
def parse_args():
    ...
    p.add_argument("--reactome_mapping", type=Path,
                   default=root / "data" / "Reactome" / "final" / "percentile90_min10_matched_combined_pathways.csv")
    p.add_argument("--ckpt_path", type=Path,
                   default=root / "data" / "weights" / "meta_trained_model_fc.pth")

    # Experiment params (optional but recommended for reproducibility)
    ...
    p.add_argument("--diseases", type=str,
                   default="idiopathic_pulmonary_fibrosis,HBV-HCC,cirrhosis,ipf_ssc,IgA_nephropathy")
    
    # Which weights to load before evaluation
    # - "pathway_maml": load pathway MAML checkpoint (default)
    # - "pathway_mlp": load pathway MLP weights
    p.add_argument("--init_mode", type=str, choices=["pathway_maml", "pathway_mlp"], default="pathway_maml")
...
class GeneExpressionDataset(Dataset):
    def __init__(self, data_folder, training_files, target_files, gene_to_index):
        ...
        for target_file in target_files:
            ...
            # (Optional) Sanity check after reordering
            assert all(self.gene_to_index[self.gene_order[i]] == i for i in range(len(self.gene_order)))
...
def save_query_lig_to_csv(disease_name, task_idx, query_indices, y_query_cpu, attr_matrix, pathway_to_index, out_root: Path):
    # Save per-sample pathway LIG for the query set of one task
    out_dir = out_root / disease_name
...
def save_query_gene_ig_to_csv(
    disease_name, task_idx, query_indices, y_query_cpu, gene_attr_matrix, gene_to_index, out_root: Path):
    # Save per-sample gene IG for the query set of one task
    out_dir = out_root / disease_name
    out_dir.mkdir(parents=True, exist_ok=True)

    # Gene index -> name mapping file (once per disease, idempotent)
    idx2gene = {idx: name for name, idx in gene_to_index.items()}
    map_path = out_dir / "gene_index_to_name.csv"
    if not map_path.exists():
        with open(map_path, "w", newline="") as f:
            w = csv.writer(f)
    ...
    save_path = out_dir / f'query_GENE_IG_task{task_idx:03d}.csv'
...
def save_mean_pathway_scores(mean_attr, disease_name, label, pathway_to_index, out_dir: Path):
    # Save averaged pathway scores per label group
    out_dir.mkdir(parents=True, exist_ok=True)
    ...
    csv_path = out_dir / f"{disease_name}_{label_tag}_pathway_scores.csv"
...
class MAML:
    def __init__(self, model, dataset, inner_lr, meta_lr, K=1, inner_steps=1, tasks_per_meta_batch=3, L2_lambda=3e-4, use_second_order=True,
                 ckpt_path: Path = None, results_dir: Path = None, pathway_to_index = None, init_path: Path=None, init_mode: str="pathway_maml"):
        """
        Initialize the MAML (Model-Agnostic Meta-Learning) object.

        Args:
            model (nn.Module): The model to be meta-trained (e.g., PathwayMAMLModel).
        ...
        self.pathway_to_index = pathway_to_index
    ...
    def evaluate(self, K, target_files, metatest_tasks, metatest_steps, metatest_lr):
        ...
        if self.init_mode == "pathway_mlp":
            load_path = self.init_path
        else:
        ...
        for target_file in target_files:
            disease_name = os.path.basename(os.path.dirname(target_file))
            pr_auc_list, auc_list = [], []
            
            # # make disease-specific directory once
            # base_dir = self.results_dir / "biomarker"
            # out_dir = base_dir / disease_name
            # out_dir.mkdir(parents=True, exist_ok=True)
            
            # LIG/IG output roots (switch by init_mode)
            if self.init_mode == "pathway_mlp":
                lig_root = self.results_dir / "LIG_raw_random_init"
                ig_root  = self.results_dir / "IG_raw_random_init"
            else:
                lig_root = self.results_dir / "LIG_raw"
                ig_root  = self.results_dir / "IG_raw"
            ...
            for task_idx in range(metatest_tasks):
                ...
                attr_mat = LIG(
                    ...
                    pathway_to_index=self.pathway_to_index,
                    n_steps=100,
    ...
    def save_results_to_csv(self, predictions, targets, overall_avg_pr_auc, overall_std_pr_auc, overall_avg_auc, overall_std_auc, results_by_disease):
        ...
        """
        save_directory = self.results_dir
        save_directory.mkdir(parents=True, exist_ok=True)
        filename = save_directory / "evaluation_results_by_diseases.csv"
        
        # Open the file in write mode
        with open(str(filename), mode='w', newline='') as file:
...
def main():
    ...
    # Set device inside main to avoid side effects when importing this file as a module
    global device
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device}")

    # Use CLI-provided seed for reproducibility
    torch.manual_seed(args.seed)
    np.random.seed(args.seed)

    # Parse disease list from CSV string
    ...
    # Build dataset paths
    tcga_dir = data_dir / args.tcga_subdir
    ...
    data_folder = tcga_dir
    training_files = sorted([p.name for p in data_folder.iterdir() if p.is_file()])

    mapping_file_path = args.reactome_mapping

    # Load gene-pathway mapping using the first target file
    gene_pathway_mapping, gene_to_index, pathway_to_index, target_ID = \
        GenePathwayMapping.load_gene_pathway_mapping(str(mapping_file_path), str(target_files[0]))

    # Keep Dataset code unchanged by passing strings (minimal edits)
    ...
    pathway_dim = len(pathway_to_index)
    ...
    # Save random init checkpoint under data/weights
    init_path = data_dir / "weights" / "random_init.pth"
    init_path.parent.mkdir(parents=True, exist_ok=True)
    torch.save(model.state_dict(), str(init_path))
